...
void spi_Init()
{
 /*1*///установка аппаратных пинов SPI (MOSI,SS,SCK) на выход (MISO на вход) на пин MISO в PORTx установить подт€жку к "+" питани€
      //установить пин SS на выход(чтобы во врем€ работы он случайно не стал входом)иначе ћ  перейдет в режим SLAVE
 ...
 /*4*///частоту тактировани€ работы SPI устанавливаем в универсальной функции настройки под каждое устройство spi_SetClockDivider(SPI_ClockDivider divider);
...
{
	//„астоту тактировани€ устанавливаем комбинацией битаов SPR1, SPR0 в регистре SPCR и бита SPI2X в регистре SPSR
	switch(divider)
	{
...
{
 //¬ыставл€ем режим работы SPI шины:
 // 1).(пол€рность сигнала SCK->сигнал в покое либо "0" либо "1")-> выставл€етс€ битом CPOL в регистре SPCR 
 // 2).(считывание значени€)-> либо по переднему фронту либо по заднему фронту меандра тактовой частоты -> выставл€етс€ битом CPHA в регистре SPCR
...
{
  // ”становка выполн€етс€ битом DORD в регистре SPCR
  // ¬ыставл€ем то, какой бит будет пересылатьс€ первым:
